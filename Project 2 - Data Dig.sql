...
SELECT AVG(price) AS avg_price, MAX(price) AS max_price, MIN(price) AS min_price FROM sales;


SELECT product, AVG(price) AS avg_price, MAX(price) AS max_price, MIN(price) AS min_price FROM sales GROUP BY product;


SELECT MAX(city) AS most_profitable_city, state, country FROM sales;


SELECT MIN(city) AS least_profitable_city, state, country FROM sales;


SELECT transaction_date, product, price, city, state, country FROM sales WHERE country = "United states" OR country = "Canada" GROUP BY country, state, city;


SELECT SUM(price) AS store_sales, transaction_date, product, price, city, state, country FROM sales WHERE country = "United states" OR country = "Canada" GROUP BY country, state HAVING store_sales > 5000;
